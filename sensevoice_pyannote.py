"""
Fusion Speaker Diarization: SenseVoice + PyAnnote
Kết hợp:
- SenseVoice (FunAudioLLM) cho ASR
- PyAnnote WeSpeaker-ResNet34 cho speaker embedding

Usage:
    python sensevoice_pyannote.py --audio_file path/to/audio.wav
"""
...
try:
    ...
    print("  pip install funasr pyannote.audio torch soundfile")
    print(f"\nOriginal error: {e}")
    sys.exit(1)

# Configuration
SAMPLE_RATE = 16000
DEVICE = "cpu"  # SenseVoice works best on CPU
LANGUAGE = "auto"
...
class SenseVoicePyAnnoteDiarization:
    """Fusion system: SenseVoice ASR + PyAnnote Speaker Embedding"""
    
    def __init__(self):
        print(f"🔄 Initializing SenseVoice + PyAnnote Fusion System")
        ...
        self.embedding_model = PretrainedSpeakerEmbedding(
            ...
            device=torch.device("cpu")  # PyAnnote on CPU for stability
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="SenseVoice + PyAnnote Fusion Diarization",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process single audio file
  python sensevoice_pyannote.py --audio_file audio.wav
  
  # Specify output file
  python sensevoice_pyannote.py --audio_file audio.wav --output result.json
        """
    )
    ...
    # Initialize system
    try:
        system = SenseVoicePyAnnoteDiarization()
    except Exception as e:
    ...
    if args.output:
        ...
        output_path = f"sensevoice_pyannote_{audio_basename}_{timestamp}.json"
